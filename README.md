...
This code uses a feed forward neural network to decide which sub goal to desire(decision network) whilst a shunting model is used to navigate towards that sub goal. The decision network outputs values for each object in the environment where: 1=desired, 0=ignored and -1=avoided. The shunting model propagates desired object values across the environment to desired or ignored states, preventing an animat from running into an avoided object.To train the decision network, tournament selection is used, splitting animats into 3 per tournament, where the worse performer is replaced by an offspring of the other 2. Animats are trained untill 80% can complete the environment. Animats are trained of 3 environments, each more difficult than the last.
